...
namespace NodeVector {
  ...
  NativeVector::NativeVector(NativeVector const& ovec) : Vector(static_cast<Vector>(ovec)) {}
  ...
  NAN_METHOD(NativeVector::GetBuffer)
  {
    ...
    Local<Object> buffer( NanNewBufferHandle( (uint32_t) vector->Size() * sizeof(vec_dim_pair_t) ) );

    vector->CopyToArray( (vec_dim_pair_t *)node::Buffer::Data(buffer), vector->Size() );
  ...
  NAN_METHOD(NativeVector::Add)
  {
    ...
    if ( args.Length() > 0 ) {
      if ( ! vector->Add( args[0] ) )
        return NanThrowTypeError("first argument should be Buffer or a Vector an Array");
    }
